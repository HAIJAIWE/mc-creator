...
- [x] **Step 1:** 更新 `fabric-adapter.test.ts` 中 `player_right_click_block` 的断言（现断言 `contains('TODO: 注册 player_right_click_block 事件')` → 改为断言 `contains('UseBlockCallback.EVENT.register')`），先跑红
- [x] **Step 2:** 实现 `fabricEventRegistration` 的 5 个新 case（UseBlockCallback / UseItemCallback / AttackBlockCallback / PlayerPickupItemCallback ×2）
- [x] **Step 3:** 新增测试覆盖 item_pickup / item_use / player_left_click 注册代码
- [x] **Step 4:** 跑 `packages/core` 测试全绿
...
- [x] **Step 1:** 抽取 `conditionCheckCode` / `actionExecuteCode` 到 fabric-adapter（protected），check/execute 方法体引用
- [x] **Step 2:** 按上表实现子集；测试先行：为 is_day / send_message / give_item / heal 各加断言（`expect(content).toContain('level.isDay()')` 等）→ 红 → 绿
- [x] **Step 3:** neoforge-adapter 复用（extend 或 copy 公共实现），更新 neoforge 测试
- [x] **Step 4:** 全量 `packages/core` 测试绿
...
- [x] **Step 1:** 主流程 `compileNodeGraph` 中，遍历 `graph.subgraphs` 时额外收集 loop 引用子图内的 condition 节点 → 加入 `spec.conditions`（带 `conditionId`，供 adapter 生成 `check_<id>` 方法）
- [x] **Step 2:** `compileLoopBody` 中 condition 节点 → 输出 `if (check_<id>(event)) { ... }`（读取子图内 action/code 节点作为 if 体），移除占位注释；无下游时输出空 if + warning
- [x] **Step 3:** 把 loop 子图内 procedure 节点合并进主流程 `procedures` 输出（复用 `compileProcedureNode` 语义：从子图节点收集 condition/action），`compileLoopBody` 中 procedure 节点 → `procedure_<name>(event);`
- [x] **Step 4:** 更新 `e2e-pipeline.test.ts:656` 附近断言（现断言 `contains('TODO')`）→ 断言 `procedure_` 调用与 `check_` 条件存在；新增 loop body 含 condition + procedure 的端到端用例
- [x] **Step 5:** 跑 `apps/desktop` 低代码相关测试绿
...
- [x] **Step 1:** `BiomeSpec` 新增 `surfaceRule: z.record(z.unknown()).optional()`（完整 surface_rule JSON，优先级最高）
- [x] **Step 2:** `generateBiome` 移除 `surface_builder` 输出；surface_rule 取值顺序：`spec.surfaceRule`（若为对象）→ `surfaceBuilder` 字符串映射：`'minecraft:grass'` → `{"type":"minecraft:grass"}`、`'minecraft:stone'` → `{"type":"minecraft:stone"}`、其余 JSON 字符串安全解析（`safeJsonParse`）→ 无值时不输出字段
- [x] **Step 3:** 更新 datapack-generator.test.ts：断言 `surface_builder` 的用例改为断言 `surface_rule`；新增 surfaceRule 对象直通用例
- [x] **Step 4:** `packages/shared` + `packages/core` 测试绿
...
- [x] **Step 2:** `pnpm -r test` — 全部通过
- [x] **Step 3:** `pnpm lint` — 0 errors（既有 warnings 忽略）
- [x] **Step 4:** 按 Task 分组提交（commit message 遵循 conventional commits）：
...
- [x] Fabric 事件注册：5 个新事件映射真实 Fabric API，生成代码可编译
- [x] condition/action：≥8 个常用类型产出真实 Java 逻辑（is_day/send_message/give_item/heal 等有断言）
- [x] loop 循环体：condition/procedure 生成真实调用，无「not yet compiled」占位
- [x] biome：无 `surface_builder`，有 `surface_rule`（含对象直通）
