...
describe('PurecodeWorkspace', () => {
  ...
  it('点击保存按钮触发 window.alert（占位提示）', () => {
    render(<PurecodeWorkspace />);
    const alertSpy = vi.spyOn(window, 'alert').mockImplementation(() => {});
    fireEvent.click(screen.getByRole('button', { name: /保存当前文件/ }));
    expect(alertSpy).toHaveBeenCalledTimes(1);
    expect(alertSpy).toHaveBeenCalledWith(expect.stringContaining('ModMain.java'));
  ...
  it('只读模式下保存按钮禁用且文件下拉禁用', () => {
    render(<PurecodeWorkspace readOnly />);
