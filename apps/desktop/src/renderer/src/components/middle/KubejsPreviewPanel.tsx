...
export function KubejsPreviewPanel() {
  ...
  const conflicts = useMemo(() => {
    ...
    const tooltipIds = new Map<string, number>();
    const registryIds = new Map<string, number>();
    for (const r of kj.recipes) {
      recipeIds.set(r.id, (recipeIds.get(r.id) ?? 0) + 1);
    }
    for (const t of kj.tags) {
      tagIds.set(t.id, (tagIds.get(t.id) ?? 0) + 1);
    }
    for (const e of kj.events) {
      eventIds.set(e.id, (eventIds.get(e.id) ?? 0) + 1);
    }
    for (const t of kj.tooltips) {
      tooltipIds.set(t.itemId, (tooltipIds.get(t.itemId) ?? 0) + 1);
    }
    for (const r of kj.registry) {
      registryIds.set(r.id, (registryIds.get(r.id) ?? 0) + 1);
    }
    return {
      duplicateRecipeIds: Array.from(recipeIds.entries())
        .filter(([, count]) => count > 1)
        .map(([id, count]) => ({ id, count })),
      duplicateTagIds: Array.from(tagIds.entries())
        .filter(([, count]) => count > 1)
        .map(([id, count]) => ({ id, count })),
      duplicateEventIds: Array.from(eventIds.entries())
        .filter(([, count]) => count > 1)
        .map(([id, count]) => ({ id, count })),
      duplicateTooltipIds: Array.from(tooltipIds.entries())
        .filter(([, count]) => count > 1)
        .map(([id, count]) => ({ id, count })),
      duplicateRegistryIds: Array.from(registryIds.entries())
        .filter(([, count]) => count > 1)
        .map(([id, count]) => ({ id, count })),
    };
  }, [kj]);
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-mc-surface">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {activeTab === 'metadata' && <MetadataView kj={kj} />}
        {activeTab === 'export' && (
          <ExportView
            kj={kj}
            stats={stats}
...
// ===== 元数据视图 =====
function MetadataView({ kj }: { kj: KubejsSpec }) {
  const rows: { label: string; value: string }[] = [
    ...
    {
      label: '语言列表',
      value: Object.keys(kj.lang ?? {}).join(', ') || '—',
    },
  ];
